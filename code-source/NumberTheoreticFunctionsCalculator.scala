...
object NumberTheoreticFunctionsCalculator {
  ...
  def isPrime(num: Long): Boolean = Math.abs(num) match {
    ...
    case n => var primeFlag = true
      val threshold = Math.sqrt(n)
      var p = 3L
      while (p < threshold && primeFlag) {
        primeFlag = n % p != 0
        p += 2
      }
      primeFlag
  ...
  def euclideanGCD(a: Long, b: Long): Long = {
    ...
    if (Math.abs(currA) < Math.abs(currB)) {
      currA = Math.abs(b)
      currB = Math.abs(a)
    }
    while (currB != 0) {
  ...
  def divideOutUnits(num: AlgInt): AlgInt = new UnaryInt(0)
